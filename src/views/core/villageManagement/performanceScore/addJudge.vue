      ...
      <el-table  class="el-judge-table" size="small"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-radio-group v-model="scope.row.assessLevel" v-show="scope.row.postName==null||scope.row.postName.indexOf('党组织书记')==-1">
              <el-radio label="优秀">优秀</el-radio>
              <el-radio label="合格">合格</el-radio>
              <el-radio label="不合格">不合格</el-radio>
            </el-radio-group>
...
<script>
    import {getUserList,save} from "@/api/dailyOffice/judge";
    export default {
        name: "addJudge",
      props: {
        valueArr: {
      ...
      methods:{
        change(){
          this.Perform.tableData.forEach(_=>{
            if(_.postName==null||_.postName.indexOf("党组织书记")==-1){
              _.assessLevel=this.Perform.status1
            }
          })
        },
        ...
        submit(){
          ...
          let flag=false;
          this.Perform.tableData.forEach(_=>{
            if((_.postName==null||_.postName.indexOf("党组织书记")==-1)&&!_.assessLevel){
              flag=true;
            }
          })
          if(!flag) {
            this.Perform.judgesUserList=this.Perform.tableData;
              ...
              if (valid) {
                save(that.Perform).then((res) => {
                  if (res.data.success) {
                    ...
                    that.$router.push({path: "/core/villageManagement/performanceScore/judge"});
                  }
                });
              }
            })
