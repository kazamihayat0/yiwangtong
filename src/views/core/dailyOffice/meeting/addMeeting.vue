<template>
  <basic-container>
    ...
    >
      <meeting-look :theMeetId="meetingForm.theMeetId"></meeting-look>
...
import meetingLook  from "./meetingLook.vue";
export default {
  components: { baiduMap ,form1,form2,meetingLook},
  data() {
    return {
      rules1:{
  ...
  methods: {
    meetDetail(){
      this.look=true;
      this.meetingForm.theMeetId=this.meetingForm.theMeetId;
    },
      ...
      if(this.meetingForm.meetTypeId===""){
        ...
        for (let i = 0; i < this.meetTypeIdOptions.length; i++) {
          if (this.meetingForm.meetTypeId == this.meetTypeIdOptions[i].id) {
            ...
            this.meetingForm.theMeetId=this.meetTypeIdOptions[i].id
          }
        }
