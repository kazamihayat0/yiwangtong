      ...
      >
        ...
        <template slot-scope="scope" slot="menu">
          <el-button
            ...
            icon="el-icon-edit"
            size="small"
            v-if="userInfo.level==2&&scope.row.status=='未评价'"
...
<script>
  import {getList} from "@/api/dailyOffice/judge";
  ...
  export default {
          ...
          column: [
            {
              ...
              action: "/api/ygj/judges/importData"
            },
            {
              label: '模板下载',
              prop: 'excelTemplate',
    ...
    methods: {
      ...
      handleExport() {
        this.$confirm("是否导出上级研判数据?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          window.open(`/api/ygj/judges/exportJudge?${this.website.tokenHeader}=${getToken()}`);
        });
      },
      ...
      addScore() {
        this.$router.push({ path: "/performance/addJudge" });
      },
      searchReset() {
        this.params = {};
        this.search={};
