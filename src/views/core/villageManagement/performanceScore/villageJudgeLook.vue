  ...
  <basic-container>
    <el-form  :rules="rules" ref="Perform" label-width="110px" :model="Perform" size="small">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="乡镇" prop="areaName">
            <el-input v-model="Perform.streetName" style="width: 96%" disabled placeholder="请输入"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
...
<script>
  import {villageDetail} from "@/api/dailyOffice/judge";
  export default {
    name: "villageJudgeLook",
    props: {
      valueArr: {
        default: () => {
    ...
    ,
    mounted(){
      villageDetail(this.$route.query.id).then(res=>{
        this.Perform=res.data.data;
        this.Perform.tableData=res.data.data.villageJudgesAssessList;
      })
    }
  }
</script>
