    ...
    <div v-for="(item,index) in data">
      <div class="warn-list" v-if="item.eventType==11">
        <div>
          ...
          <div class="warn-flex warn-line-h"><div class="warn-gray">上月打卡</div><div  class="warn-ma">{{ item.syNum }}次/人</div></div>
          <div class="warn-flex warn-line-h"><div class="warn-gray">应打卡</div><div  class="warn-ma">{{ item.ydkNum  }}次/人</div></div>
      ...
      <div>
        ...
        <div class="warn-flex warn-line-h"><div class="warn-gray">上周打卡</div><div  class="warn-ma">{{item.szNum }}次</div></div>
        <div class="warn-flex warn-line-h"><div class="warn-gray"> 本月打卡</div><div  class="warn-ma">{{item.byNum }}次</div></div>
        <div class="warn-flex warn-line-h" v-if="item.isPush!=0"><div class="warn-gray">已通知</div><div  class="warn-ma">{{item.pushInfo }}</div><div  class="warn-ma">{{item.postName}}</div></div>
      </div>
      <div style="line-height:150px;height: 150px ">
        <el-button
          :type="index==3?'info':'primary'"
          @click.stop="addComprehensive(item.userId)"
        >添加至不合格村干部
        </el-button>
      ...
      <div class="warn-list" v-if="item.eventType==22">
      <div>
        <div class="warn-flex">
          <div><img :src="item.avatar" style="border-radius: 50%"  width="50px" height="50px"/></div>
          <div>
        ...
        <div class="warn-flex warn-line-h"><div class="warn-gray">上月打卡</div><div  class="warn-ma">{{item.syNum }}次</div></div>
        <div class="warn-flex warn-line-h"><div class="warn-gray"> 应打卡</div><div  class="warn-ma">{{item.ydkNum }}次</div></div>
        <div class="warn-flex warn-line-h"  v-if="item.isPush!=0"><div class="warn-gray">已通知</div><div  class="warn-ma">{{item.pushInfo }}</div><div  class="warn-ma">{{item.postName}}</div></div>
      </div>
      <div style="line-height:150px;height: 150px ">
        <el-button
